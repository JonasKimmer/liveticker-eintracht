...
\item
  ...
  Bulk-Generierungsendpunkt und Provider-Override ermöglicht systematische,
  reproduzierbare Modellvergleiche. Statistische Vergleichsmetriken
  (Cliff's Delta, Bootstrap-CI, Cohen's Kappa) sind als Infrastruktur
  implementiert und für externe Folgestudien mit mehreren Ratern ausgelegt
  (vgl. Abschnitt~\ref{methodische-einordnung}).
\item
  ...
  (vgl. Abschnitt 7.3 für die ausführliche Diskussion).
\end{enumerate}
...
Die folgenden Abschnitte ordnen die zentralen Limitationen des Systems
hinsichtlich ihrer Implikationen ein.
...
Textqualität. Im Design Science Research ist die Evaluation durch den
Entwickler ein verbreitetes Vorgehen bei initialen Artefaktiterationen
(Hevner et al.~2004), begrenzt jedoch die externe Validität der
qualitativen Ergebnisse. Die implementierte Cohen's-Kappa-Metrik ist
bewusst als Infrastruktur für eine Folgestudie mit externen Ratern
angelegt.

Die eingeschränkte Stichprobengröße relativiert die Generalisierbarkeit
der quantitativen Ergebnisse. Insbesondere Randsituationen --- torloses
...
Das Polling-Modell stellt für den Einzelbetrieb keine Einschränkung dar, würde aber bei steigender
...
Die fehlende Authentifizierung ist im Kontext eines internen
Redaktionswerkzeugs vertretbar, schließt aber einen offenen Mehrbenutzerbetrieb ohne
...
Eine automatisierte CI-Pipeline fehlt im aktuellen Systemstand. Die Testsuite ist lokal ausführbar, wird jedoch nicht bei jedem Commit automatisch verifiziert, was die Regressionssicherheit bei parallelen Entwicklungsaktivitäten einschränkt. Die Integration eines CI-Diensts wäre ein direkter Folgeschritt ohne architektonische Voraussetzungen.
...
(z. B. „TOOOOR!!!") --- stellen besondere Anforderungen an die
...
Die Evaluationsergebnisse (vgl. Abschnitt~\ref{qualitative-analyse-der-generierten-texte}) zeigen, dass die
...
niedrige Temperatur (vgl. Abschnitt 7.4.1) sind Gegenmaßnahmen, deren
...
Abschnitt~\ref{evaluation-der-betriebsmodi}). Das Risiko besteht in unkontrollierten
...
Der \texttt{coop}-Modus ist der intendierte Produktivbetrieb, weil er
die beiden zentralen Anforderungen balanciert: die Geschwindigkeit der
KI-Generierung und die publizistische Verantwortung des Redakteurs.
Der \texttt{auto}-Modus ist für journalistische Kontexte zu riskant, da
Halluzinationen ohne Prüfung veröffentlicht werden. Der
\texttt{manual}-Modus eliminiert den Zeitdruckvorteil der KI vollständig.
Der \texttt{coop}-Modus kombiniert beides: Das LLM liefert den Entwurf
innerhalb von Sekunden, der Redakteur prüft und gibt mit TAB frei oder
verwirft mit ESC. Die Tastatursteuerung ist dabei entscheidend --- sie
sichert den Entscheidungsdurchsatz, ohne den Redakteur visuell zu
überlasten.

Da \texttt{coop}-Einträge dieselbe LLM-Generierung durchlaufen wie
\texttt{auto}-Einträge und zusätzlich redaktionell geprüft werden, ist
ihre Textqualität strukturell mindestens gleich. Im Evaluationszeitraum
wurden im \texttt{coop}-Modus keine Einträge nach Freigabe
zurückgezogen, was auf eine wirksame Qualitätsstufe hindeutet. Ob
Entwürfe vor der Freigabe inhaltlich editiert wurden, wurde nicht
systematisch erfasst. Bearbeitungen sind über das Frontend jedoch
jederzeit möglich.
...
aktivieren. Diese gestaffelte Einführung adressiert den in Kapitel 2.1
...
\subsection{Few-Shot-Prompting als
...
durchführbar, da alle produktiv generierten Einträge mit aktiven
Stilreferenzen erstellt wurden und eine Kontrollgruppe ex-post nicht
konstruierbar ist. Qualitativ zeigt die Evaluation
...
\subsection{Temperatur und
Determinismus}\label{temperatur-und-determinismus}

Die gewählte Temperatur von 0,5 für die Textgenerierung und 0,1 für
...
\subsection{Kontext-Aufbereitung}\label{kontext-aufbereitung}

Die sieben spezialisierten Context-Builder (vgl. Abschnitt~\ref{llm-service-prompt-aufbau-und-provider-dispatch})
...
identifizierten drei Anforderungsdimensionen (Zeitdruck, Mehrsprachigkeit,
