...
\texttt{build\_match\_context()} erstellt das Kontext-Dictionary für den LLM-Prompt aus Teamnamen, aktuellem Stand, Matchzustand, Spielminute und Liga. Wie die Kontextanreicherung (Stufe~2 der Pipeline) je nach Ereignistyp verläuft, ist in Abschnitt~\ref{pipeline-mechanik-und-idempotenz} beschrieben.
...
Die fünfstufige LLM-Pipeline verarbeitet eingehende Events vom Datenimport bis zum gespeicherten Ticker-Eintrag. Sie gliedert sich in Pipeline-Überblick (Abschnitt~\ref{pipeline-mechanik-und-idempotenz}), Kontextanreicherung (Abschnitt~\ref{kontextanreicherung-pipeline}), Idempotenz (Abschnitt~\ref{idempotenz-und-generierungssteuerung}) und Generierungssteuerung (Abschnitt~\ref{generierungssteuerung}).

\subsection{Pipeline-Überblick}\label{pipeline-mechanik-und-idempotenz}

Die Pipeline durchläuft fünf Stufen: Event-Import, Kontextanreicherung, Prompt-Aufbau, LLM-Aufruf und Persistenz. Die Stufen~1 und~2 sind in \texttt{ticker\_service} (Abschnitt~\ref{ticker-service-domuxe4nenlogik}), Stufen~3 und~4 im \texttt{LLMService} (Abschnitt~\ref{llm-service-prompt-aufbau-und-provider-dispatch}) implementiert.
...
\subsection{Kontextanreicherung}\label{kontextanreicherung-pipeline}

Die Kontextanreicherung (Stufe~2) verläuft je nach Ereignistyp unterschiedlich. Für Live-Events (\texttt{goal}, \texttt{card} u.\,a.) erzeugt \texttt{build\_context\_data()} ein Dict aus Teamnamen und Spielstand, das \texttt{build\_context\_str()} anschließend als \texttt{ctx\_match\_info}-Block in den Prompt einbettet.

Für synthetische Pre-Match-Events (z.\,B. \texttt{pre\_match\_h2h}) liefert \texttt{synthetic.data} die JSONB-Nutzdaten direkt — also exakt die Daten, die WF07 zuvor aus der Football-API importiert und in \texttt{synthetic\_events} persistiert hat. \texttt{build\_context\_str()} wählt anhand des \texttt{event\_type} den passenden Formatter (\texttt{ctx\_h2h}, \texttt{ctx\_injuries}, \texttt{ctx\_standings} u.\,a.) und übersetzt die strukturierten JSON-Daten in lesbare Prompt-Zeilen. Abbildung~\ref{fig:ki-generierungspipeline} zeigt den vollständigen Ablauf beider Pfade vom Ereignis-Import bis zum gespeicherten Ticker-Eintrag.
...
\subsection{Idempotenz}\label{idempotenz-und-generierungssteuerung}
...
Diese Schutzprüfung fehlt im Endpunkt \texttt{POST\ /generate-synthetic} für Spielphasen-Events bewusst (vgl. Abschnitt~\ref{prompt-engineering-strategie} zur asymmetrischen Idempotenz-Strategie).

Der Endpunkt \texttt{POST /generate-match-phases/\{match\_id\}} ist ein Catch-up-Mechanismus: Während eines laufenden Spiels generiert n8n WF14 Spielphasen-Einträge über \texttt{POST /generate-synthetic} bei jedem Phasenübergang. Öffnet ein Redakteur jedoch ein Spiel erstmalig in einer späteren Phase, können einzelne Phasen-Drafts fehlen — der Frontend-Hook \texttt{useMatchTriggers} löst dann \texttt{generate-match-phases} aus und gleicht fehlende Einträge mit Standardminuten nach.

Das erzeugte Phasenset ist spielzustandsabhängig: Im Standardfall werden vier Phasen erzeugt (Anpfiff, Halbzeit, zweite Halbzeit, Abpfiff); bei Verlängerung ergänzt der Router \texttt{EXTRA\_PHASES}, bei Elfmeterschießen zusätzlich \texttt{PENALTY\_PHASES}. Bei laufenden Spielen wird der Abpfiff-Eintrag übersprungen, um keinen vorzeitigen FullTime-Text zu erzeugen. \texttt{get\_or\_create\_flush()} stellt sicher, dass jede Phase idempotent auf genau einem \texttt{SyntheticEvent}-Datensatz aufbaut.

\subsection{Generierungssteuerung}\label{generierungssteuerung}

Der initiale Status des erzeugten Ticker-Eintrags wird über den Aufrufparameter \texttt{auto\_publish} gesteuert, den n8n aus dem konfigurierten Spielmodus ableitet (vgl. Abschnitt~\ref{status-lifecycle-und-betriebsmodi}):
...
Damit wird die Modus-Logik zu einem reinen Aufrufparameter des Backends (vgl. Abschnitt~\ref{n8n-designprinzipien}).
