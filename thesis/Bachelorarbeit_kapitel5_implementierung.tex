...
\texttt{app/main.py} ist der einzige Einstiegspunkt. Er enthält zwölf Router-Includes: elf fachliche Bereiche unter \texttt{/api/v1} sowie den WebSocket-Router \texttt{/ws/media}. Außerdem konfiguriert er CORS-Middleware und bindet statische Dateien (Thumbnail-Cache) ein. Da der Ticker-Bereich auf drei eigenständige Sub-Router-Dateien aufgeteilt ist (vgl. Abschnitt~\ref{api-router-endpunktstruktur}), umfasst \texttt{api/v1/} physisch 13 Router-Dateien.

Beim Start führt ein \texttt{lifespan}-Context-Manager \texttt{alembic upgrade head} aus und spielt ausstehende Migrationen automatisch ein, bevor der erste Request eintrifft. Die Datenbankverbindung wird per FastAPI Dependency Injection in jeden Router injiziert: \texttt{core/database.py} stellt einen \texttt{get\_db()}-Generator bereit, der pro Anfrage eine Session öffnet und am Anfrageende garantiert schließt.
...
Stammdaten-Entitäten kombinieren drei Schlüsselfelder (\texttt{id}, \texttt{external\_id}, \texttt{uid}) und werden bei Reimports per \texttt{ON CONFLICT DO UPDATE} aktualisiert. Abhängige Entitäten tragen \texttt{ondelete="CASCADE"}-Bindungen (vgl. Abschnitt~\ref{schemadesign-prinzipien}).
...
Drei zusammengesetzte Indizes optimieren die häufigsten Abfragen: \texttt{ix\_ticker\_match\_status} für publizierte Einträge je Spiel, \texttt{ix\_ticker\_match\_phase} für Phase-Filter und \texttt{ix\_ticker\_event\_id} für den Deduplizierungs-Check per Event.
...
Der \texttt{ticker\_service} kapselt die Domänenlogik für KI-Einträge und koordiniert Datenbankabfragen, Kontextaufbau und den LLM-Aufruf (vgl. Abschnitt~\ref{interne-schichtstruktur} zur Layering-Einschränkung).

Der Service umfasst vier Kernfunktionen. \texttt{score\_at\_event()} berechnet den Spielstand zum Zeitpunkt eines Torereignisses: Da die Football-API den kumulativen Stand liefert, muss der Stand \emph{vor} dem aktuellen Event rekonstruiert werden. Eigentore werden korrekt verarbeitet, da API-Football \texttt{team\_id} stets auf den Nutznießer des Tores setzt.

\texttt{build\_match\_context()} erstellt das Kontext-Dictionary für den LLM-Prompt aus Teamnamen, aktuellem Stand, Matchzustand, Spielminute und Liga. Wie die Kontextanreicherung (Stufe~2 der Pipeline) je nach Ereignistyp verläuft, ist in Abschnitt~\ref{kontextanreicherung-pipeline} beschrieben.
...
Für die Fehlerbehandlung unterscheidet der Service zwei Backoff-Strategien: Bei Rate-Limit-Fehlern (HTTP~429) greift lineares Backoff (\texttt{30\,s / 60\,s}), bei transienten Fehlern exponentielles Backoff (\texttt{1\,s / 2\,s}), jeweils bis zu \texttt{LLM\_RETRY\_ATTEMPTS} (3) Wiederholungen. Danach wird die ursprüngliche Ausnahme weitergereicht. Unabhängig vom Provider überschreibt \texttt{call\_llm()} für \texttt{ef\_whitelabel}-Instanzen das Stilprofil \texttt{neutral} intern auf \texttt{euphorisch} (vgl. Abschnitt~\ref{partner-team-konzept-und-white-label-steuerung}).


\section{KI-Generierungspipeline}\label{ki-generierungspipeline}

Die fünfstufige LLM-Pipeline verarbeitet eingehende Events vom Datenimport bis zum gespeicherten Ticker-Eintrag (Event-Import, Kontextanreicherung, Prompt-Aufbau, LLM-Aufruf, Persistenz). Sie gliedert sich in Spielstandberechnung und Kontextanreicherung (Abschnitt~\ref{kontextanreicherung-pipeline}), Prompt-Design und Stilkonditionierung (Abschnitt~\ref{prompt-design-stilkonditionierung}) sowie Idempotenz und Generierungssteuerung (Abschnitt~\ref{idempotenz-und-generierungssteuerung}).
...
Die Trennung von Daten- und Aktions-Context verhindert unnötige Re-Renders (vgl. Abschnitt~\ref{architekturprinzipien}): \texttt{EntryEditor} konsumiert nur den Aktions-Context und re-rendert damit nicht bei Änderungen an \texttt{tickerTexts}. Alle Contexts exportieren einen typsicheren Zugriffs-Hook, der bei Verwendung außerhalb des Providers einen klaren Fehler wirft.
...
\texttt{useMatchData} aggregiert die drei Polling-Hooks \texttt{useMatchCore}, \texttt{useMatchEvents} und \texttt{useMatchTicker} und stellt ihre Daten über \texttt{TickerDataContext} bereit. Die Polling-Intervalle und Bedingungen sind in Abschnitt~\ref{frontend-polling} beschrieben.
...
\begin{itemize}\tightlist
\item \textbf{UPSERT-Strategien:} Alle Insert-Operationen verwenden \texttt{ON CONFLICT DO UPDATE} oder \texttt{DO NOTHING} (vgl. Abschnitt~\ref{n8n-designprinzipien}).
\item \textbf{Filter-Knoten:} Vor LLM-Triggern steht jeweils ein Filterknoten, der eine Datenbankzeile nur dann durchlässt, wenn sie eine gültige \texttt{id} enthält \emph{und} als Neu-Insert markiert ist (\texttt{is\_new~=~true}). UPSERT-aktualisierte Events passieren den Filter nicht und lösen keinen redundanten LLM-Aufruf aus (vgl. Abschnitt~\ref{n8n-designprinzipien}).
...
\item \textbf{Mode-Propagation:} \texttt{ticker\_mode} wird als \texttt{auto\_publish}-Flag an den Generierungsendpunkt übergeben und bestimmt den Publikationsstatus des erzeugten Eintrags (vgl. Abschnitt~\ref{n8n-designprinzipien}).
\end{itemize}
