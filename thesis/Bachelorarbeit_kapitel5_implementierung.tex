...
CORS-Middleware und bindet statische Dateien (Thumbnail-Cache) ein. Beim Start führt ein \texttt{lifespan}-Context-Manager \texttt{alembic upgrade head} aus, sodass ausstehende Migrationen automatisch eingespielt werden, bevor der erste Request eintrifft. Die Datenbankverbindung wird per FastAPI Dependency Injection in jeden Router injiziert: \texttt{core/database.py} stellt einen \texttt{get\_db()}-Generator bereit, der für jede Anfrage eine Session öffnet und am Anfrageende garantiert schließt:
...
Die Anwendung stellt drei Metaendpunkte bereit:
...
\texttt{GET\ /health} & Liveness-Check (gibt immer \texttt{healthy} zurück) \\
\texttt{GET\ /health/db} & Datenbankverbindungscheck (\texttt{healthy}/\texttt{degraded}) \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
Events \& Ticker & 15.000 ms & Live, Abpfiff (\texttt{resolvePollingInterval}) \\
Events \& Ticker & 20.000 ms & Alle anderen Spielstatus (PreMatch, Pause) \\
Match-Refresh & 30.000 ms & Einmalig beim Spielwechsel \\
Match-Sync & 60.000 ms & Live, PreMatch, ToBeConfirmed \\
\end{longtable}
}

Die \texttt{resolvePollingInterval}-Utility differenziert das Intervall je nach Spielstatus: Im Live-Modus sowie bei Spielende (\texttt{FullTime}) beträgt es 15.000\,ms, in allen anderen Zuständen (PreMatch, Pause u.\,a.) 20.000\,ms. Für einen Produktiveinsatz mit vielen gleichzeitigen Nutzern könnte das PreMatch-Intervall weiter auf 30.000\,ms oder mehr erhöht werden, um die Serverlast außerhalb der Spielphasen zu senken.


\section{n8n Workflow-Implementierung}\label{n8n-workflow-implementierung}

Die n8n-Workflow-Implementierung setzt die in Abschnitt~\ref{workflow-klassen} konzipierten Workflow-Klassen als 17 versionierte JSON-Exporte unter \texttt{n8n/} um (Knotentypen vgl. Abschnitt~\ref{n8n-technologiewahl}). Sie gliedert sich in einen Workflow-Überblick (Abschnitt~\ref{workflow-ueberblick}), den Events-LLM-Workflow (Abschnitt~\ref{events-llm-workflow}), den Prematch-Import-Workflow (Abschnitt~\ref{prematch-import-workflow}), den Matchphasen-Workflow (Abschnitt~\ref{matchphasen-workflow}), Zusammenfassung und Medien (Abschnitt~\ref{zusammenfassung-und-medien-workflow}) sowie übergreifende Implementierungsmuster (Abschnitt~\ref{n8n-implementierungsmuster}).

\subsection{Workflow-Überblick}\label{workflow-ueberblick}

Abbildung~\ref{fig:workflow-timeline} zeigt, welche Workflows zu welchem Zeitpunkt eines Spieltags aktiv sind. Phasengesteuerte Workflows (WF14, WF13) feuern einmalig bei Zustandsübergängen; ereignis- und zeitbasierte Workflows (WF09, WF15, WF16) laufen während beider Halbzeiten kontinuierlich.
...
\item \textbf{Filter-Knoten:} Vor event-basierten LLM-Triggern (09, 14) steht ein Filterknoten, der nur Rows mit einer \texttt{id} (erfolgreicher DB-Insert) durchlässt; Workflow~13 (Zusammenfassung) und Workflow~15 (Live-Stats-Monitor) rufen den LLM-Provider direkt per HTTP-Request auf, nicht über das FastAPI-Backend.
...
Unit-Tests decken Utility-Funktionen, Hooks und UI-Komponenten ab. Sieben Hook-Tests sichern das Zustandsmanagement isoliert von der UI ab, darunter \texttt{useMatchEvents}, \texttt{useMatchTicker} und \texttt{useLiveMinute}. Fünf Komponenten-Tests prüfen kritische UI-Elemente: \texttt{EntryEditor}, \texttt{PublishedEntry}, \texttt{CommandPalette}, \texttt{RightPanelComponents} und \texttt{ErrorBoundary}. Der \texttt{parseCommand}-Parser ist mit 29 Testfällen besonders umfangreich abgesichert: Als kritische manuelle Eingabeschicht muss er sämtliche Slash-Command-Varianten, Fehleingaben und Kantenfälle zuverlässig verarbeiten. Ein Fehler hier würde sich direkt in falsch klassierten Ticker-Einträgen niederschlagen.

\subsection{Backend-Tests (pytest)}\label{impl-backend-tests}

API-Integrationstests prüfen alle Endpunkte gegen eine echte PostgreSQL-Testinstanz mit transaktionalem Rollback. Nach jedem Test wird die Datenbank auf den Ausgangszustand zurückgesetzt, sodass Tests voneinander isoliert und in beliebiger Reihenfolge ausführbar sind. Repository- und Service-Unit-Tests ergänzen die Integrationstests auf Schichtebene. Der Verzicht auf Datenbank-Mocks in diesen Schichten ist bewusst: Mock-Implementierungen riskieren Divergenzen zum realen Schema, die gerade bei Schemaänderungen während der Entwicklung unbemerkt bleiben. Der LLM-Service wird hingegen gemockt, da externe LLM-Aufrufe nicht deterministisch und in automatisierten Läufen nicht reproduzierbar sind. Ergänzend enthält \texttt{evaluation\_metrics.py} statistische Hilfsfunktionen (Cliff's Delta, Bootstrap-Konfidenzintervall, Cohen's Kappa u.\,a.) als Evaluationsinfrastruktur für die KI-Textbewertung in Kapitel~\ref{evaluation}.

\subsection{End-to-End-Tests (Playwright)}\label{impl-e2e-tests}

Browser-seitige Smoke-Tests in \texttt{e2e/liveticker.spec.ts} validieren grundlegende Nutzerinteraktionen: App-Laden ohne JavaScript-Fehler, Sichtbarkeit des Länder-Dropdowns, Öffnen und Schließen des Keyboard-Shortcuts-Modals über den \texttt{?}-Button sowie ein Viewport-Test bei 375\,×\,812\,px (iPhone~SE) als Regressionsschutz gegen Layout-Brüche auf kleinen Bildschirmen (N5). Testmetriken, Coverage-Kennzahlen und TypeScript-Typsicherheitswerte sind in Abschnitt~\ref{technische-qualituxe4tssicherung} dokumentiert.


\section{Deployment}\label{deployment}

Das Deployment setzt das in Abschnitt~\ref{deployment-konzept} beschriebene Konzept als produktiv erreichbare Infrastruktur um. Alle vier Komponenten werden deklarativ über \texttt{render.yaml} im Repository-Branch \texttt{deploy/render-v2} konfiguriert.
