...
Stammdaten-Entitäten kombinieren drei Schlüsselfelder (\texttt{id}, \texttt{external\_id}, \texttt{uid}); abhängige Entitäten tragen \texttt{ondelete="CASCADE"}-Bindungen und Stammdaten werden bei Reimports per \texttt{ON CONFLICT DO UPDATE} aktualisiert (vgl. Abschnitt~\ref{schemadesign-prinzipien}).
...
Als Standardmodell im Deployment wird \texttt{google/gemini-2.0-flash-lite-001} über OpenRouter eingesetzt (vgl. Abschnitt~\ref{multi-provider-architektur} für die Begründung der Modellwahl). Die folgende Tabelle zeigt alle Provider der Prioritätskette mit ihren Standardmodellen. Die Temperatur für Textgenerierung ist einheitlich \texttt{LLM\_TEMPERATURE = 0.5}, für Übersetzungen \texttt{LLM\_TRANSLATION\_TEMPERATURE = 0.1}.
...
\texttt{rejected}-Einträge verbleiben mit ihrem Status in der Datenbank (vgl. Abschnitt~\ref{status-lifecycle-und-betriebsmodi}). Die Evaluation der Textqualität ist in Kapitel~\ref{evaluation} dokumentiert.
