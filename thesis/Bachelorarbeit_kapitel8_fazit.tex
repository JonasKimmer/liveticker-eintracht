...
\subsection{Konstruktiver Teil: Konzeption und
Implementierung}\label{konstruktiver-teil}

Der erste Teil der Forschungsfrage fragt, ob ein solches System
realisierbar ist. Die Antwort ist eindeutig positiv: Das entwickelte
System realisiert die dreischichtige Architektur aus n8n-Datenbeschaffung,
FastAPI/PostgreSQL-Backend und React/TypeScript-Frontend vollständig. Die
16 n8n-Workflows versorgen das System automatisiert mit Echtzeit-Daten;
die LLM-Pipeline (Gemini 2.0 Flash Lite via OpenRouter) generiert
Ticker-Einträge für alle definierten Event-Typen in drei Stilprofilen und
mehreren Sprachen. Die White-Label-Architektur und das dreistufige
Betriebs\-modus-Konzept (\texttt{auto}, \texttt{coop}, \texttt{manual})
sind produktionsnah realisiert und im Cloud-Deployment erreichbar. Alle
23 definierten funktionalen, nicht-funktionalen und architektonischen
Anforderungen werden erfüllt (vgl. Abschnitt 6.6); 352 automatisierte
Tests und eine TypeScript-Typenabdeckung von 95,84 \% belegen die
technische Reife.

\subsection{Qualitative Dimension: Journalistische
Qualität}\label{qualitative-dimension-journalistische-qualituxe4t}

Die journalistische Qualität wurde entlang der drei in der Forschungsfrage definierten Kriterien operationalisiert. \textbf{Korrektheit}: Die generierten Texte erreichen einen Ø-Wert von \textbf{4,6 / 5}. In 15 von 16 bewerteten Einträgen (94~\%) wurden alle verfügbaren Fakten (Spieler, Team, Minute, Ergebnis) korrekt wiedergegeben (Hauptevaluation, N~=~16, deutschsprachig). Die ergänzende Evaluation mit synthetischem Kontext (N~=~9) bestätigt diesen Befund und erweitert ihn um bislang fehlende Event-Typen (Rote Karte, Phasen-Events). Die Pre-Match-Schutzregel verhindert zuverlässig die Erfindung von Live-Spielszenen; eine Wettempfehlung (1~von~16 Einträgen, 6~\%) stellt eine inhaltliche Halluzination geringerer Schwere dar. \textbf{Tonalität}: Mit einem Ø-Wert von \textbf{4,1 / 5} erzeugen die drei Stilprofile deutlich unterscheidbare Textstile. Der euphorische Modus erzielt die stärksten qualitativen Ergebnisse; das neutrale Profil zeigt die größte Inkonsistenz (19~\% der Einträge zu emotional formuliert), bedingt durch die starken euphorischen Few-Shot-Referenzen. \textbf{Verständlichkeit}: Mit \textbf{4,3 / 5} erfüllen die generierten Texte die linguistischen Anforderungen des Liveticker-Genres (Kapitel~2.5) zuverlässig --- kurze Satzkonstruktionen, Präsenskonstruktionen, idiomatische Ausrufe und konzeptionelle Mündlichkeit sind durchgehend vorhanden.

Die Qualitätsbewertung stützt sich auf drei Evaluationsebenen: die in
Abschnitt 6.3.6 dokumentierte Hauptevaluation (N = 16,
Entwicklerselbstbewertung), das Experteninterview mit einem
professionellen Sportredakteur von Eintracht Frankfurt (vgl. Abschnitt
6.3.6) sowie die LLM-as-Judge-Evaluation (N = 20, Claude Sonnet 4.6 als
unabhängiger Bewerter, Abschnitt 6.3.7). Die ergänzende Evaluation
bestätigt den Gesamtdurchschnitt mit 4,5 / 5 und repliziert dieselben
Fehlerklassen (semantischer Framing-Fehler, Phrasenwiederhol im
euphorischen Profil).

\subsection{Latenzpotenzial: Publikationsbeschleunigung}\label{latenzpotenzial}

Der zweite Teil der Forschungsfrage fragt nach dem nachweisbaren
Latenzpotenzial. Da ein kontrolliertes Feldexperiment nicht durchführbar
war, basiert die Abschätzung auf gemessenen LLM-Latenzen (Median 859 ms,
P95 2.047 ms, vgl. Abschnitt 6.5.1) und der implementierten
Systemarchitektur. Im \texttt{auto}-Modus ergibt sich daraus eine
geschätzte TTP von \textbf{3,4--5,9 s}; im \texttt{coop}-Modus von
\textbf{15--30 s} (zuzüglich redaktioneller Prüfzeit). Gegenüber der
manuellen Baseline von \textbf{30--120 s} (Kapitel 2.1) entspricht dies
einem geschätzten \textbf{Reduktionsfaktor von 5× bis 35×} im
\texttt{auto}-Modus. Die Abschätzung ist konservativ und methodisch
transparent (vgl. Abschnitt 6.4); ein Feldtest mit echten
Redakteuren bleibt als Folgestudie vorgesehen (vgl. Abschnitt 8.3.2).
...
\textbf{n8n-Workflow-Tests}: Die 16 n8n-Workflows (vgl. Kapitel 6.7)
