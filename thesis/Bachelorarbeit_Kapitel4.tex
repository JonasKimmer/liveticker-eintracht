...
Das System ist als \textbf{cloud-fähige Architektur} konzipiert, die
alle Spiele der konfigurierten Wettbewerbe automatisch verarbeitet ---
von Datenimportierung über KI-Textgenerierung bis zur Publikation. Die
Datenbeschaffungs- und Automatisierungsschicht (n8n) wird in der
aktuellen Implementierung lokal über ngrok betrieben; für
Produktivbetrieb müsste die n8n-Instanz auf einer Cloud-Plattform
deployed werden. Es ist eine serviceorientierte, dreischichtige Architektur,
...
Die \textbf{Logikschicht} basiert auf FastAPI (Python), während
PostgreSQL als \textbf{Persistenzschicht} dient. Sie stellt die REST-Schnittstellen bereit,
...
Die \textbf{Präsentationsschicht} ist als White-Label-Frontend mit React
und TypeScript realisiert. Sie bildet den Redaktionsworkflow für
Auswahl, Bearbeitung und Publikation ab und bleibt dabei
mandantenfähig konfigurierbar (z. B. Vereinsname, Stilprofil, Sprache,
Farben und Instanzpfad, hinterlegt in
\texttt{config/whitelabel.ts}).
...
\caption{Dreischichtige Systemarchitektur mit Kommunikationspfaden}
...
if missing''} als Leitlinie: Das Frontend liest zunächst den
vorhandenen Datenbestand über REST-Polling; bei fehlenden oder
veralteten Daten können passende n8n-Webhooks ausgelöst werden. Im
Live-Betrieb können Importe zusätzlich redaktionell manuell getriggert
werden. Dadurch werden externe API-Aufrufe minimiert und redundante
Importe vermieden.

\begin{figure}[H]
\centering
\DiagramCommunicationFlow
\caption{Kommunikationsfluss nach dem „read first, trigger if missing"-Prinzip}
...
\item
  ...
  \item
    \textbf{Match-Kontext-Trigger} --- spielbezogene Datenimporte nach
    Matchauswahl (Events, Lineups, Statistiken), die mehrere
    Generierungsschritte versorgen
  \item
    \textbf{Generierungs-Trigger} --- ereignisbasierte KI-Textprozesse
    sowie Matchphasen-Trigger (Anpfiff, Halbzeit, Abpfiff) und
    Pre-Match-Generierung
  \end{enumerate}
\item
  \textbf{WebSocket \texttt{/ws/media}} --- Echtzeit-Push für
  latenzkritische Medieninhalte (ScorePlay-Bilder); Ticker- und
  Matchdaten bleiben bewusst im Polling-Modell.
\end{itemize}
...
Verein, für den die White-Label-Instanz konfiguriert ist --- den
jeweiligen Gegnern sowie neutralen Spielen ohne Beteiligung des
Partner-Teams. Diese Unterscheidung wird über einen konfigurierbaren
Teamnamen-Abgleich umgesetzt (\texttt{teamKeyword}), der sowohl in den
n8n-Workflows als auch im Frontend gegen den Teamnamen geprüft wird.
...
\item
  ...
  \texttt{ef\_whitelabel}-Instanzpfad mit situationsadaptivem Stil
  (euphorisch bei eigenen Erfolgen, neutral bei gegnerischen Toren,
  kritisch bei Rückständen); alle anderen Spiele den
  \texttt{generic}-Pfad mit neutralem Stil.
\item
  \textbf{UI-Anpassung}: Das Frontend blendet vereinsspezifische
  Bereiche (Social-Media-Panels: YouTube, Twitter/X, Instagram)
  ausschließlich bei erkannten Partner-Team-Spielen ein.
\item
  \textbf{Erweiterte Kontextdaten}: Für Tormomente des Partner-Teams
  werden zusätzlich Mediendaten (z. B. ScorePlay-Bilder) über den
  WebSocket-Kanal bereitgestellt und in den redaktionellen Kontext
  eingebettet.
\end{itemize}
...
\centering
\DiagramDataModel
\caption{Vereinfachtes ER-Diagramm der zentralen Datenbankentitäten}
\label{fig:datamodel}
\end{figure}
...
\centering
\DiagramTickerStates
\caption{Status-Lifecycle der Ticker-Einträge}
\label{fig:tickerstates}
\end{figure}
...
& Bei leeren Daten. Hinweis: Der Wettbewerb (Competition) wird für den
Kontext abgerufen, aber nicht direkt persistiert; die Persistenz erfolgt
implizit über nachgelagerte Abhängigkeitsketten. \\
...
\caption{Modularer Prompt-Aufbau für die LLM-Textgenerierung}
...
\item
  ...
  nach \texttt{event\_id}, gefiltert nach Spielphase und
  Vorberichtserstattung, sortiert nach Spielminute.
\item
  ...
  Social-Media-Panels werden ausschließlich bei erkannten
  Partner-Team-Spielen eingeblendet.
\item
