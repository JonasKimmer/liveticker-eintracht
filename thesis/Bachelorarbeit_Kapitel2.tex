...
Erstens bietet der Ticker-Editor weder KI-Unterstützung noch technische Stilführung. Trotz vorhandener redaktioneller Richtlinien für den Eintracht-Ton kann das System diese weder einfordern noch vorschlagen (vgl. Abschnitt~\ref{experteninterview-leitfaden}).

Zweitens wird der Spielstand über numerische Eingabefelder manuell gepflegt (Abbildung~\ref{fig:altsystem}, links unten). Obwohl der Verein externe Datenquellen (Opta, DFBnet) für Livedaten nutzt, fehlt eine automatische Synchronisation dieser Feeds mit der Liveticker-Oberfläche. Die Daten sind grundsätzlich zuverlässig, bleiben aber anfällig für Verzögerungen und Fehlzuordnungen, etwa bei kurzfristigen Aufstellungsänderungen oder instabiler Internetverbindung im Auswärtsstadion. Dadurch bleiben manuelle Korrekturen durch Redakteure essenziell (vgl. Abschnitt~\ref{experteninterview-leitfaden}).

Drittens fehlt eine strukturierte Medienintegration. Das System bietet lediglich einen einfachen „Bild auswählen"-Button ohne Social-Media-Anbindung, Galeriefunktion oder automatischen Medien-Import (z.\,B. ScorePlay, YouTube, Instagram).
...
Fünftens sind Live-Statistiken und audiovisuelle Einbettungen im Altsystem nicht vorhanden, obwohl Aufstellungsdaten laut Experteninterview bereits automatisiert übertragen werden. Im Experteninterview wurde hierzu berichtet, dass audiovisuelle Inhalte und Live-Statistiken im Altsystem derzeit nicht darstellbar sind (vgl. Abschnitt~\ref{experteninterview-leitfaden}).

Sechstens erhöht die hohe Systemkomplexität durch zahlreiche Sonderlösungen die Fehleranfälligkeit im laufenden Betrieb, insbesondere für Redakteure mit geringerer Systemroutine (vgl. Abschnitt~\ref{experteninterview-leitfaden}).

Diese Defizite spiegeln die drei in Abschnitt~\ref{problemstellung-und-zielsetzung} identifizierten Anforderungsdimensionen direkt wider: operativer Zeitdruck durch vollständig manuelle Produktion, fehlende Mehrsprachigkeit und eingeschränkte White-Label-Fähigkeit. Wie Kapitel~\ref{systemkonzeption} und Abschnitt~\ref{anforderungsabgleich-soll-ist-vergleich} zeigen, adressiert das entwickelte System diese sechs Defizitbereiche systematisch.
