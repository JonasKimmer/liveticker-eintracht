...
Die Arbeit leistet drei Beiträge zum aktuellen Forschungsstand. Als architektonischen Beitrag demonstriert sie eine produktionsnahe Referenzarchitektur für KI-gestützte Echtzeit-Textgenerierung mit modularer Provider-Abstraktion, Few-Shot-Prompting aus einer kuratierten Stilreferenz-Datenbank und dreistufigem Betriebsmodus-Konzept. Als methodischen Beitrag stellt die Evaluationsinfrastruktur mit Bulk-Generierungsendpunkt, Provider-Override und statistischen Vergleichsmetriken (Cliff's Delta, Bootstrap-CI, Cohen's Kappa) systematische und reproduzierbare Modellvergleiche bereit. Als praktischen Beitrag adressiert die White-Label-Architektur mit instanzspezifischer Stilsteuerung den in Kapitel 2.3 beschriebenen Bedarf von Vereinen als eigenständige Medienproduzenten.
...
Die 16 n8n-Workflows (vgl. Kapitel 6.7, „Keine Tests für
...
niedrige Temperatur (vgl. Abschnitt~5.1) sind Gegenmaßnahmen, deren
...
Die gewählte Temperatur von 0,3 für die Textgenerierung und 0,1 für Übersetzungen ist ein Kompromiss zwischen sprachlicher Varianz und Vorhersagbarkeit. Eine niedrigere Temperatur würde die Halluzinationsrate weiter senken, aber die Texte formelhafter machen --- ein Zielkonflikt, der im Genre Liveticker (das von Überraschung und sprachlicher Kreativität lebt) besonders relevant ist. Die sieben spezialisierten Context-Builder (vgl. Abschnitt~5.1) strukturieren die Fakten vor der Übergabe an das LLM und reduzieren damit die Wahrscheinlichkeit von Halluzinationen, da das Modell bereits aufbereitete Faktenblöcke erhält statt unstrukturierte Rohdaten. Die Wirksamkeit zeigt sich insbesondere bei Pre-Match-Einträgen, wo die Faktengrundlage (z.~B. Verletzungslisten, H2H-Statistiken) klar abgegrenzt ist.


\section{Implikationen für den
Sportjournalismus}\label{implikationen-fuxfcr-den-sportjournalismus}

\subsection{Veränderung der
...
der Textproduktion zur Textredaktion: Statt unter
...
Die Zukunftsperspektive wird im Experteninterview (vgl.~Interviewleitfaden, Block~4, Fragen IF16--IF18) adressiert. Aus Sicht des Experten sind insbesondere die Konfigurierbarkeit des Stils und die Möglichkeit der Laufzeitumschaltung zwischen den Modi praxisrelevante Stärken. Technische Voraussetzung für eine echte Multi-Vereins-Skalierung ist die Mandantenfähigkeit auf Datenbankebene, da die aktuelle Architektur beide Instanzen (\texttt{ef\_whitelabel}, \texttt{generic}) in einem Schema hält. Die Skalierbarkeits- und White-Label-Bewertung stützt sich auf die in Kapitel~5 dokumentierte Systemarchitektur und die Deployment-Erfahrung mit der bestehenden \texttt{ef\_whitelabel}-Instanz. Konkrete Erweiterungsszenarien --- mehrsprachige Ausgabe, Erweiterung der Datenbasis, Nutzerstudie mit Redakteuren --- sind in Abschnitt~8.3 beschrieben.

\subsection{Ethische Überlegungen}\label{ethische-uxfcberlegungen}
...
Darüber hinaus berührt der Einsatz automatisierter Textgenerierung drei
weitere ethische Dimensionen, die im Rahmen dieser Arbeit nicht
abschließend bewertet werden, jedoch für eine Produktiveinführung
relevant sind: \textbf{Arbeitsmarktimplikationen} --- die in
Abschnitt~7.5.1 diskutierte Neukalibrierung der Redakteursrolle birgt
langfristig auch das Risiko eines Stellenabbaus, da geringere manuelle
Produktionsleistung erforderlich wird (vgl. Bluhm \& Schäfer 2023,
S.~35). \textbf{Datenschutz} --- Spieler- und Vereinsdaten werden zur
Textgenerierung an externe LLM-Provider übermittelt; eine
DSGVO-konforme Datenverarbeitungsvereinbarung mit den verwendeten
Providern ist für den Produktivbetrieb zwingend erforderlich.
\textbf{Haftung} --- im \texttt{auto}-Modus werden faktisch fehlerhafte
Einträge (z.~B. falsche Torschützen) ohne menschliche Prüfung
publiziert; die publizistische Verantwortung und eventuelle rechtliche
Haftung verbleiben beim Betreiber des Systems, nicht beim LLM-Provider.


\section{Synthese}\label{synthese}

Die Diskussion entlang der fünf Dimensionen --- Abgrenzung zum Stand der Technik, Limitationen, Betriebsmodi, Prompt-Architektur und Implikationen --- ergibt ein konsistentes Gesamtbild: Das System adressiert die identifizierten Problemdimensionen durch nachweisbare Mechanismen; verbleibende Schwächen (Stil-Inkonsistenz, fehlende Authentifizierung, eingeschränkte Stichprobengröße) sind klar lokalisiert. Die Beantwortung der Forschungsfrage und die modus-spezifische Gesamtempfehlung erfolgen in Kapitel~8.2; der konkrete Erweiterungsausblick in Kapitel~8.3.
