...
\subsection{Auto-Modus: Geschwindigkeit auf Kosten der
...
Latenzpotenzial-Abschätzung in Abschnitt~6.4). Das Risiko besteht in unkontrollierten
...
\subsection{Coop-Modus: Der intendierte
...
zurückgezogen. Dies deutet darauf hin, dass die redaktionelle Prüfung als
Qualitätsschicht greift; ob Entwürfe vor der Freigabe inhaltlich editiert
wurden, wurde jedoch nicht systematisch erfasst (vgl. Abschnitt~6.7),
weshalb eine abschließende Bewertung der Filterleistung des
\texttt{coop}-Modus einer Folgestudie vorbehalten bleibt. Das
Experteninterview (vgl. Abschnitt~6.3.4) bestätigt diese
Einschätzung: Der befragte Sportredakteur bewertete den
\texttt{coop}-Modus als den praktisch relevantesten Betriebsmodus, da
er Geschwindigkeit mit redaktioneller Kontrolle verbindet. Die
...
\subsection{Manual-Modus: Status quo als
...
Laufzeit per API-Call --- könnte eine schrittweise Einführung in
Redaktionen unterstützen: Neue Nutzer könnten im \texttt{manual}-Modus
beginnen, über den \texttt{coop}-Modus Vertrauen in die KI-Qualität
aufbauen und für unkritische Standardereignisse schließlich den
\texttt{auto}-Modus aktivieren. Ob diese Graduierung in der Praxis so
durchlaufen wird, ist empirisch nicht belegt; sie würde jedoch den
in Kapitel 2.1 beschriebenen Akzeptanzbedarf bei der Einführung
KI-gestützter Werkzeuge in redaktionelle Workflows adressieren.
...
Die Diskussion der Evaluationsergebnisse entlang fünf Dimensionen
--- Abgrenzung zum Stand der Technik, methodische und technische
Limitationen, Betriebsmodi, Prompt-Architektur sowie Implikationen für
den Sportjournalismus --- ergibt ein
