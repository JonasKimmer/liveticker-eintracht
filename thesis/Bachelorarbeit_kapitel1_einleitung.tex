...
\item
  ...
  und in stilistisch kohärente Kurztexte überführen — unter einem
  Zeitdruck, der manuelle Skalierung strukturell begrenzt
  (vgl. Hauser 2008, S.~3--4; Abschnitt~\ref{anforderungsanalyse-am-beispiel-eintracht-frankfurt}).
\item
  ...
  bereitzustellen, ohne ausreichendes mehrsprachiges Personal vorzuhalten.
\item
...
Aus der in Abschnitt~\ref{problemstellung-und-zielsetzung} entwickelten Zielsetzung ergibt sich folgende Forschungsfrage, die der vorliegenden Arbeit zugrunde liegt:

\begin{quote}
\emph{Lässt sich ein hybrides KI-gestütztes Redaktionssystem für die Liveticker-Erstellung im Profifußball realisieren, das Echtzeit-Verarbeitung, Mehrsprachigkeit und White-Label-Betrieb unterstützt, und inwiefern erfüllt ein solches System journalistische Qualitätsanforderungen hinsichtlich Korrektheit, Tonalität und Verständlichkeit?}
\end{quote}

Die Frage verbindet eine \textbf{Realisierbarkeits-} mit einer \textbf{Qualitätsdimension}. Der erste Teil ist im Sinne des Design Science Research (Hevner et al.~2004) konstruktiv angelegt: Konzeption, Architekturentscheid und prototypische Realisierung eines lauffähigen Artefakts. Der zweite Teil ist evaluativ: die empirische Prüfung, ob das System journalistische Mindestanforderungen an Korrektheit, Tonalität und Verständlichkeit erfüllt. Die Forschungsfrage wird in Abschnitt~\ref{methodisches-vorgehen} methodisch operationalisiert und in Kapitel~\ref{evaluation} vollständig beantwortet.
...
ergänzt durch ein \textbf{leitfadengestütztes Experteninterview} mit einem
professionellen Sportredakteur zur Validierung der Systemeignung im
redaktionellen Arbeitskontext sowie durch ein \textbf{LLM-as-Judge}-Verfahren
(Zheng et al.~2023) zur skalierbaren automatisierten Qualitätsbewertung.
...
werden in Kapitel~\ref{systemkonzeption} und~\ref{implementierung} beschrieben.

Dennoch bestehen Einschränkungen hinsichtlich der Authentifizierung
(kein vollständiges Rollenmodell implementiert, da das System als
intern genutztes Redaktionswerkzeug in einer kontrollierten Umgebung
betrieben wird und ein vollständiges Berechtigungskonzept über den
Scope der Forschungsfrage hinausginge), der Evaluationsunabhängigkeit
(der Autor ist gleichzeitig Entwickler und Bewerter) sowie der
thematischen Reichweite (ausschließlich Fußball-Liveticker), die in
Kapitel~\ref{diskussion} kritisch reflektiert werden.
...
\item
  \textbf{Kapitel~7 -- Diskussion:} Einordnung der Evaluationsergebnisse in den Stand der Technik, kritische Reflexion der Limitationen, Experteninterview-Auswertung sowie ethische und journalistische Implikationen.
\item
