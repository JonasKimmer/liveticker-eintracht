...
Das Backend folgt einer fünfschichtigen Struktur. \textbf{Router} (\texttt{app/api/v1/}) empfangen HTTP-Anfragen; neben Ressourcen-Routern für Stammdaten und Events ist der Ticker-Router in drei Sub-Router aufgeteilt: \texttt{ticker\_crud} für Lese- und Statusoperationen, \texttt{ticker\_generate} für KI-Generierungsendpunkte und \texttt{ticker\_batch} für Bulk-Evaluation und Übersetzung. Die \textbf{Service-Schicht} (\texttt{app/services/}) umfasst zwei Dienste mit klar getrennten Rollen: Der \texttt{TickerService} koordiniert Kontextaufbau, Spielstandberechnung und parallele LLM-Aufrufe über einen konfigurierbaren Semaphore; der \texttt{LLMService} abstrahiert die Provider-Kommunikation und trägt keine Datenbankabhängigkeit. \textbf{Repositories} (\texttt{app/repositories/}) kapseln den gesamten Datenbankzugriff je einer Klasse pro Entität, die jeweils eine SQLAlchemy-Session entgegennehmen. \textbf{ORM-Modelle} (\texttt{app/models/}) bilden die Tabellen auf Python-Klassen ab; \textbf{Pydantic-Schemas} (\texttt{app/schemas/}) typisieren Ein- und Ausgaben und bilden gleichzeitig die Grundlage der automatisch generierten OpenAPI-Dokumentation.

Stammdaten-Operationen folgen dem direkten Muster Router \(\rightarrow\) Repository. KI-Generierungsabläufe durchlaufen die Service-Schicht: Router \(\rightarrow\) TickerService \(\rightarrow\) Repository + LLMService. Diese Trennung ermöglicht transaktionale Testbarkeit, da Repositories mit echten Datenbanktransaktionen geprüft und nach jedem Test zurückgerollt werden können. Die Schnittstelle ist als versionierte REST-API unter \texttt{/api/v1} ausgelegt (vgl. Fielding 2000, Kap.~5); Leseoperationen wie \texttt{GET /ticker/match/\{id\}} bleiben seiteneffektfrei, KI-Generierung und Datenimport erfolgen über dedizierte Triggerendpunkte wie \texttt{POST /ticker/generate/\{event\_id\}}. Die vollständige Endpunktstruktur ist in Abschnitt~\ref{api-router-endpunktstruktur} dokumentiert.

Zwei übergreifende Robustheitsprinzipien durchziehen das Backend. Triggerendpunkte sind idempotent ausgelegt: Wiederholte n8n-Aufrufe erzeugen keine Duplikate, da der \texttt{TickerService} vor jedem LLM-Aufruf prüft, ob ein Eintrag für das auslösende Event bereits existiert. Datensätze werden nicht physisch gelöscht; Ticker-Einträge erhalten stattdessen den Status \texttt{deleted} (Soft-Delete), sodass abgelehnte und zurückgezogene Einträge für spätere Qualitätsauswertungen reproduzierbar erhalten bleiben.
...
Die Backend-Qualitätssicherung gliedert sich in drei Testebenen. Reine Unit-Tests prüfen zustandslose Logikfunktionen (Spielstandberechnung, Prompt-Kontext-Aufbau, statistische Hilfsfunktionen) ohne Datenbankabhängigkeit. API-Integrationstests bilden die hauptsächliche Schicht: \textbf{Pytest} prüft die Backend-Endpunkte sowie Repositories gegen eine echte PostgreSQL-Testinstanz, um Abweichungen zwischen ORM-Modell und realem Datenbankverhalten frühzeitig zu erkennen. LLM-Provider werden dabei durch einen deterministischen Mock-Provider ersetzt, da echte Aufrufe in automatisierten Läufen nicht reproduzierbar sind. Eine schlanke E2E-Schicht validiert kritische Nutzerpfade im Browser via \textbf{Playwright}. Frontend-Teststrategie und Werkzeugwahl (\textbf{Jest}) sind in Abschnitt~\ref{frontend-teststrategie} beschrieben; die konkreten Testzahlen und Coverage-Verteilung dokumentiert Abschnitt~\ref{impl-backend-tests}.
...
Die gesamte Zustands- und Interaktionslogik ist in wiederverwendbare Custom Hooks aufgeteilt, die Navigation, Polling, Echtzeitkommunikation und Redaktionsinteraktion abdecken. Die Hooks sind in zwei Ebenen organisiert: allgemein wiederverwendbare und feature-spezifische, um Kopplung zwischen Kernarchitektur und Ticker-Feature-Logik zu begrenzen. Zustandsänderungen mit sichtbarer Latenz — insbesondere die Modusumschaltung per PATCH-Aufruf — werden über Optimistic-Updates realisiert: Der UI-Zustand wird sofort lokal gesetzt, während die Backend-Synchronisation im Hintergrund läuft. Die White-Label-Fähigkeit ist über eine Konfigurationsschicht konzipiert, die Farben, Team-Keyword und API-Endpunkte je Instanz variiert (vgl. Abschnitt~\ref{komponentenhierarchie}).
