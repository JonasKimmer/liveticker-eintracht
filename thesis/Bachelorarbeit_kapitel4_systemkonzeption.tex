...
Die Generierungspipeline folgt einem dreistufigen Ablauf: Der \texttt{TickerService} baut zunächst den Spielkontext auf (aktueller Spielstand, Ereignisdaten, Stilprofil und Zielsprache). Der LLM-Dienst setzt daraus einen strukturierten Prompt zusammen und übergibt ihn an den konfigurierten Provider. Der generierte Text wird als Entwurf (\texttt{draft}) in der Datenbank abgelegt. Parallele Generierungsaufrufe für mehrere gleichzeitige Spiele werden über einen konfigurierbaren Semaphore koordiniert (vgl. Abschnitt~\ref{interne-schichtstruktur}).
