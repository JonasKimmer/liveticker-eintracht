...
Die Triggerarchitektur unterscheidet drei Kommunikationsmuster je nach Datenquelle und Betriebskontext.
...
Ein ergänzendes Triggermuster greift beim erstmaligen Öffnen eines Spiels, das sich bereits in einer späteren Phase befindet: Das Frontend prüft, ob Phasen-Drafts für bereits abgeschlossene Spielphasen vorhanden sind, und löst bei Fehlen einen Catch-up-Endpunkt aus, der die fehlenden Entwürfe nachträglich generiert. Dieses Muster stellt sicher, dass ein Redakteur, der ein laufendes Spiel erst in der zweiten Halbzeit öffnet, alle Phasen-Drafts vorfindet, ohne manuelle Eingriffe vorzunehmen.
...
Die interne Schichtstruktur folgt dem \textbf{Repository Pattern} (Fowler 2002, S.~322) und gliedert sich in vier Schichten. Router übernehmen die HTTP-Schicht und orchestrieren eingehende Anfragen. Dedizierte Repositories kapseln den gesamten Datenbankzugriff, sodass die Anwendungslogik frei von SQL-Details bleibt. Während ORM-Modelle die Persistenzstruktur der Datenbank abbilden, definieren Pydantic-Schemas die typisierten Verträge zwischen den Schichten.

Die Ablaufmuster unterscheiden sich je nach Komplexität der Operation. Einfache Create, Read, Update, Delete (CRUD)-Operationen folgen dem direkten Muster Router \(\rightarrow\) Repository. Komplexere Generierungs- und Orchestrierungsabläufe durchlaufen zusätzlich eine Service-Schicht: Router \(\rightarrow\) Service \(\rightarrow\) Repository. Diese Trennung ermöglicht transaktionale Testbarkeit, da Repositories in Tests mit echten Datenbanktransaktionen geprüft und nach jedem Test zurückgerollt werden können. Als explizite Layering-Einschränkung gilt dabei, dass der LLM-Service keine Datenbankabhängigkeit trägt — Datenbankzugriff erfolgt ausschließlich über die Repository-Schicht.
...
Das Layout ist vollständig responsiv ausgelegt. Auf mobilen Geräten kollabiert die Dreispalten-Ansicht auf eine Einspalten-Darstellung mit einer Bottom-Tab-Leiste, über die zwischen Ticker-Feed, Editor und Statistik-Panel gewechselt wird. Die Kommunikationsarchitektur des Frontends ist in Abschnitt~\ref{kommunikations--und-triggerarchitektur} beschrieben. Die konkreten Implementierungen sind in Abschnitt~\ref{frontend-implementierung} dokumentiert.
