...
Das Backend setzt auf ein hybrides Nebenläufigkeitsmodell. Die Service-Schicht operiert asynchron: LLM-Aufrufe und externe API-Anfragen sind I/O-gebunden und laufen über den asyncio-Eventloop parallel, ohne Threads zu blockieren. Die Datenbankschicht hingegen nutzt bewusst synchrones SQLAlchemy, da Datenbankoperationen im System kurzläufig sind und ein asynchrones ORM Komplexität ohne messbaren Latenz-Vorteil hinzufügen würde. Jeder Request erhält über FastAPIs Dependency-Injection-Mechanismus eine isolierte Session, die nach Abschluss der Anfrage geschlossen wird.

\subsection{Interne Schichtstruktur}\label{interne-schichtstruktur}

Das Backend folgt einer fünfschichtigen Struktur. Die \textbf{Router-Schicht} empfängt HTTP-Anfragen; der Ticker-Router ist nach funktionalen Bereichen — Datenzugriff, KI-Generierung und Bulk-Operationen — in Sub-Router aufgeteilt. Die \textbf{Service-Schicht} umfasst zwei Dienste mit klar getrennten Rollen: Der \texttt{TickerService} koordiniert Kontextaufbau, Spielstandberechnung und parallele LLM-Aufrufe; der \texttt{LLMService} abstrahiert die Provider-Kommunikation ohne Datenbankabhängigkeit. Die \textbf{Repository-Schicht} kapselt den gesamten Datenbankzugriff je Entität. \textbf{ORM-Modelle} bilden die Tabellen auf Python-Klassen ab; \textbf{Pydantic-Schemas} typisieren Ein- und Ausgaben und bilden die Grundlage der automatisch generierten OpenAPI-Dokumentation.

Stammdaten-Operationen folgen dem direkten Muster Router \(\rightarrow\) Repository. KI-Generierungsabläufe durchlaufen die Service-Schicht: Router \(\rightarrow\) TickerService \(\rightarrow\) Repository + LLMService. Diese Trennung ermöglicht transaktionale Testbarkeit, da Repositories mit echten Datenbanktransaktionen geprüft und nach jedem Test zurückgerollt werden können. Die Schnittstelle ist als versionierte REST-API ausgelegt (vgl. Fielding 2000, Kap.~5); Leseoperationen bleiben seiteneffektfrei, KI-Generierung und Datenimport erfolgen über dedizierte Triggerendpunkte. Die vollständige Endpunktstruktur ist in Abschnitt~\ref{api-router-endpunktstruktur} dokumentiert.

Zwei übergreifende Robustheitsprinzipien durchziehen das Backend. Triggerendpunkte sind idempotent ausgelegt: Wiederholte n8n-Aufrufe erzeugen keine Duplikate, da der \texttt{TickerService} vor jedem LLM-Aufruf prüft, ob ein Eintrag für das auslösende Event bereits existiert. Datensätze werden nicht physisch gelöscht; Ticker-Einträge erhalten stattdessen den Soft-Delete-Status, sodass abgelehnte und zurückgezogene Einträge für spätere Qualitätsauswertungen reproduzierbar erhalten bleiben.
