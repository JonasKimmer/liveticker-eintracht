...
export function useMatchCore(selectedMatchId: number | null) {
  ...
  const loadPlayers = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        teamIds.map((id) => api.fetchPlayers(id)),
      );
  ...
  const loadPrematch = useCallback(
    () => _load(api.fetchPrematch, setPrematch),
    [_load],
  );
  const loadLineups = useCallback(
    () => _load(api.fetchLineups, setLineups),
    [_load],
  );
  const loadMatchStats = useCallback(
    () => _load(api.fetchMatchStats, setMatchStats),
    [_load],
  );
  const loadPlayerStats = useCallback(
    () => _load(api.fetchPlayerStats, setPlayerStats),
    [_load],
  );
  const loadInjuries = useCallback(
    () => _load(api.fetchInjuries, setInjuries, (d) => d ?? []),
    [_load],
  );
