...
import { fetchTwitterPosts, triggerTwitterImport, deleteClip } from "../../../api";
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1rem" }}>
          ...
          <span style={{ fontFamily: "var(--lt-font-mono)", fontSize: "0.7rem", color: "var(--lt-text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            @Eintracht · Im Ticker veröffentlichen
          </span>
        </div>

        {/* Original Tweet als Vorschau */}
        <div style={{
...
function TweetCard({ post, onClick, onDelete }) {
  ...
  if (hasThumbnail) {
    return (
      ...
      >
        ...
        >
          <div style={{ position: "relative", paddingTop: "56.25%", background: "var(--lt-bg-card-2)" }}>
            <img
    ...
    >
      <button
        ...
        style={{
          ...
          borderRadius: 6, border: `1px solid ${hovered ? "var(--lt-accent)" : "var(--lt-border)"}`,
          background: hovered ? "var(--lt-bg-hover)" : "var(--lt-bg-card-2)",
          padding: "0.6rem 0.75rem", cursor: "pointer", outline: "none",
...
export const TwitterPanel = memo(function TwitterPanel({ matchId, currentMinute = 0 }) {
  ...
  const loadPosts = useCallback(async () => {
    ...
    setStatusMsg(null);
    try {
      const res = await fetchTwitterPosts();
      setPosts(res.data ?? []);
    } catch {
