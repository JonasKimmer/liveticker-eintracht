...
export const ModeSelector = memo(function ModeSelector({
  ...
  const confirmSwitch = useCallback(() => {
    ...
    setPopPos(null);
    setToast({
      text: `${cfg.icon} Modus gewechselt: ${cfg.label}`,
      color: cfg.color,
    });
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      if (e.key === "1") {
        e.preventDefault();
        requestSwitch(MODES.AUTO);
      }
      if (e.key === "2") {
        e.preventDefault();
        requestSwitch(MODES.COOP);
      }
      if (e.key === "3") {
        e.preventDefault();
        requestSwitch(MODES.MANUAL);
      }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  ...
  return (
    <>
          ...
          {Object.values(MODES).map((m) => {
            ...
            return (
              <button
                key={m}
                ref={(el) => {
                  btnRefs.current[m] = el as HTMLButtonElement;
                }}
      ...
      {pending &&
        ...
        createPortal(
          <div
            className="lt-mode-popover"
            style={{ top: popPos.top, left: popPos.left }}
          >
            <div
              className="lt-mode-popover__bar"
              style={{ background: pendingCfg.color }}
            />
            <div className="lt-mode-popover__body">
              <div className="lt-mode-popover__head">
                <div
                  className="lt-mode-popover__ico"
                  style={{ background: pendingCfg.colorDim }}
                >
                  {pendingCfg.icon}
                </div>
                <div>
                  <div className="lt-mode-popover__eyebrow">
                    Modus wechseln zu
                  </div>
                  <div
                    className="lt-mode-popover__title"
                    style={{ color: pendingCfg.color }}
                  >
                    {pendingCfg.label}
                  </div>
                </div>
              ...
              <div className="lt-mode-popover__actions">
                <button
                  className="lt-mode-popover__cancel"
                ...
                <button
                  ...
                  style={{
                    background: pendingCfg.color,
                    color: pendingCfg.textColor,
                  }}
            ...
            <div className="lt-mode-popover__shortcuts">
              <span className="lt-mode-popover__sh">
                <kbd className="lt-mode-kbd">↵</kbd> Bestätigen
              </span>
              <span className="lt-mode-popover__sh">
                <kbd className="lt-mode-kbd">Esc</kbd> Abbrechen
              </span>
            </div>
          </div>,
          document.body,
      ...
      {toast &&
        createPortal(
          <div className="lt-mode-toast">
            <span
              className="lt-mode-dot lt-mode-dot--sm"
