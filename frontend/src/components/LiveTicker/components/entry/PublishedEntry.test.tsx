...
const baseEntry = { id: 1, match_id: 1, event_type: "goal", liveTickerEventType: "goal", time: 32 };
const baseTicker = {
  id: 1,
  match_id: 1,
  source: "ai" as const,
...
describe("PublishedEntry — AI-Eintrag", () => {
  ...
  test("gibt null zurück ohne tickerText", () => {
    const { container } = render(
      <PublishedEntry entry={baseEntry} tickerText={null} />,
    );
  ...
  test("kein Aktionen-Button ohne onEdit und onDelete", () => {
    render(<PublishedEntry entry={baseEntry} tickerText={baseTicker} />);
    expect(screen.queryByTitle("Aktionen")).not.toBeInTheDocument();
  });
...
describe("PublishedEntry — Event Icons", () => {
  test.each([
    ...
    const entry = { id: 1, match_id: 1, event_type: eventType, liveTickerEventType: eventType, time: 60 };
