\section{Kapitel 7 -- Diskussion}\label{kapitel-7-diskussion}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{7.1 Einordnung in den Stand der
Technik}\label{einordnung-in-den-stand-der-technik}

\subsubsection{7.1.1 Abgrenzung zu bestehenden
...
kritisch ist (Bluhm \& Schäfer 2023, S. 33). Der
\passthrough{\lstinline!coop!}-Modus repräsentiert damit einen
Mittelweg, der die Effizienzgewinne der Automatisierung nutzt, ohne die
publizistische Verantwortung an die KI zu delegieren.
...
\subsubsection{7.1.2 Beitrag zur Forschung}\label{beitrag-zur-forschung}
...
\subsection{7.2 Kritische Reflexion}\label{kritische-reflexion}

Die in Kapitel 6.7 dokumentierten Limitationen werden im Folgenden
hinsichtlich ihrer Implikationen eingeordnet.

\subsubsection{7.2.1 Methodische
Einordnung}\label{methodische-einordnung}
...
\subsubsection{7.2.2 Technische Einordnung}\label{technische-einordnung}
...
Der \passthrough{\lstinline!useAutoPublisher!}-Hook im Frontend enthält
eine semantische Doppelung: Im Auto-Modus setzt n8n über
\passthrough{\lstinline!auto\_publish=True!} den Status bereits beim
Generieren; der Hook fängt Residualfälle auf. Diese Zweispurigkeit
entstand durch inkrementelle Entwicklung und könnte auf eine definitive
...
Der \passthrough{\lstinline!strict!}-Modus in
\passthrough{\lstinline!tsconfig.json!} ist derzeit deaktiviert, um die
inkrementelle TypeScript-Migration nicht zu blockieren. Eine
schrittweise Aktivierung von \passthrough{\lstinline!strictNullChecks!}
und \passthrough{\lstinline!noImplicitAny!} ist der naheliegende
Folgeschritt zur weiteren Härtung der Codebasis.

\subsubsection{7.2.3 Inhaltliche Reflexion}\label{inhaltliche-reflexion}
...
Die Few-Shot-Referenzen aus der
\passthrough{\lstinline!style\_references!}-Tabelle sind das primäre
Mittel, um diese stilistische Lücke zu schließen.
...
Wirksamkeit jedoch nur im \passthrough{\lstinline!coop!}-Modus durch die
redaktionelle Kontrolle vollständig abgesichert ist. Im
\passthrough{\lstinline!auto!}-Modus verbleibt ein Restrisiko
fehlerhafter Veröffentlichungen.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{7.3 Diskussion der
Betriebsmodi}\label{diskussion-der-betriebsmodi}

Die drei Betriebsmodi (\passthrough{\lstinline!auto!},
\passthrough{\lstinline!coop!}, \passthrough{\lstinline!manual!}) wurden
in Kapitel 4.3.3 konzipiert und in Kapitel 6.4 evaluiert. Im Folgenden
werden die Implikationen der Evaluationsergebnisse für den praktischen
Einsatz diskutiert.

\subsubsection{7.3.1 Auto-Modus: Geschwindigkeit auf Kosten der
Kontrolle}\label{auto-modus-geschwindigkeit-auf-kosten-der-kontrolle}

Die Stärke des \passthrough{\lstinline!auto!}-Modus liegt in der
Geschwindigkeit (vgl. TTP-Messung in Abschnitt 6.4.2). Das Risiko
besteht in unkontrollierten Halluzinationen, die ohne redaktionelle
Prüfung veröffentlicht werden. Im journalistischen Kontext, in dem
Glaubwürdigkeit eine zentrale Ressource darstellt (Beils 2023, S. 57),
ist dieser Modus daher nur für unkritische Event-Typen (z. B.
Phasenwechsel wie „Anpfiff'' oder „Halbzeit'') vertretbar.

\subsubsection{7.3.2 Coop-Modus: Der intendierte
...
\subsubsection{7.3.3 Manual-Modus: Status quo als
Vergleichsbasis}\label{manual-modus-status-quo-als-vergleichsbasis}

Der \passthrough{\lstinline!manual!}-Modus dient als Vergleichsbasis und
Rückfalloption --- etwa bei hochkritischen Ereignissen oder bei
Redakteuren, die zunächst Vertrauen in das System aufbauen möchten. Die
Laufzeit-Umschaltbarkeit per API-Call stellt sicher, dass der Wechsel
ohne Systemunterbrechung möglich ist.

\subsubsection{7.3.4 Implikationen für die
...
Redaktionen: Neue Nutzer können im
\passthrough{\lstinline!manual!}-Modus beginnen, über den
\passthrough{\lstinline!coop!}-Modus Vertrauen in die KI-Qualität
aufbauen und für unkritische Standardereignisse schließlich den
\passthrough{\lstinline!auto!}-Modus aktivieren. Diese Graduierung
adressiert den in Kapitel 2.1 beschriebenen Akzeptanzbedarf bei der
Einführung KI-gestützter Werkzeuge in redaktionelle Workflows.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{7.4 Diskussion der
Prompt-Architektur}\label{diskussion-der-prompt-architektur}

\subsubsection{7.4.1 Few-Shot-Prompting als
Stilsteuerung}\label{few-shot-prompting-als-stilsteuerung}

Die dynamische Einbindung von bis zu drei Stilreferenzen aus der
\passthrough{\lstinline!style\_references!}-Datenbanktabelle stellt
einen zentralen Designentscheid dar. Im Vergleich zu einem statischen
System-Prompt bietet dieser Ansatz zwei Vorteile:
...
\item
  ...
  (\passthrough{\lstinline!ef\_whitelabel!}
  vs.~\passthrough{\lstinline!generic!}) ermöglicht unterschiedliche
  Stilprofile für verschiedene Einsatzkontexte.
\end{enumerate}
...
konsistente Formatierungsmuster in der
\passthrough{\lstinline!ef\_whitelabel!}-Instanz --- insbesondere bei
TOOOOR-Konvention und Minutenformat. Der Effekt ist sichtbar, aber
statistisch nicht isoliert messbar.

\subsubsection{7.4.2 Temperatur und
...
\subsubsection{7.4.3 Kontext-Aufbereitung}\label{kontext-aufbereitung}
...
\subsection{7.5 Implikationen für den
Sportjournalismus}\label{implikationen-fuxfcr-den-sportjournalismus}

\subsubsection{7.5.1 Veränderung der
...
\subsubsection{7.5.2 Skalierbarkeit und
White-Label}\label{skalierbarkeit-und-white-label}

Die White-Label-Architektur (\passthrough{\lstinline!ef\_whitelabel!}
vs.~\passthrough{\lstinline!generic!}) adressiert den in Kapitel 2.3
beschriebenen strukturellen Wandel der Vereine zu eigenständigen
Medienproduzenten. Ein einzelnes System kann --- durch
...
erfordern. Für Vereine mit begrenzten Redaktionsressourcen senkt dies
die Einstiegshürde in eine professionelle Liveticker-Berichterstattung.
...
mit der bestehenden \passthrough{\lstinline!ef\_whitelabel!}-Instanz;
eine externe Nutzerstudie ist in Abschnitt 8.3.2 vorgesehen.

\subsubsection{7.5.3 Ethische
Überlegungen}\label{ethische-uxfcberlegungen}
...
Herkunft jedes Eintrags im Feld \passthrough{\lstinline!source!}
(\passthrough{\lstinline!"ai"!}, \passthrough{\lstinline!"manual"!}),
legt die Kennzeichnung gegenüber dem Endnutzer aber nicht offen. Für
einen produktiven Einsatz wäre eine Kennzeichnungspflicht --- etwa durch
ein dezentes Label wie „KI-unterstützt'' --- zu diskutieren,
insbesondere im Kontext der EU-KI-Verordnung (AI Act, EU 2024/1689),
deren Transparenzpflichten für KI-generierte Texte (Art. 50 Abs. 4)
gemäß Art. 113 ab August 2026 gelten.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{7.6 Synthese}\label{synthese}
...
Die zentrale Designentscheidung --- der
\passthrough{\lstinline!coop!}-Modus als primärer Betriebsmodus ---
erweist sich aus dieser Diskussion heraus als strukturell richtig: Er
balanciert die Effizienzgewinne der KI mit der publizistischen
Verantwortung des Redakteurs und adressiert damit den in Kapitel 1.2
formulierten Zielkonflikt zwischen Geschwindigkeit und Qualität. Die
Verschiebung der Redakteursrolle von der Textproduktion zur Textkuration
ist keine Umgehung journalistischer Sorgfalt, sondern ihre
Neukalibrierung unter den Bedingungen der Echtzeit-Berichterstattung.
