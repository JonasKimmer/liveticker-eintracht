...
Für das vorliegende System werden drei kompakte Sprachmodelle eingesetzt: **GPT-4o mini** (OpenAI, Modell-ID `gpt-4o-mini`), **Gemini 2.0 Flash Lite** (Google, Modell-ID `gemini-2.0-flash-lite-001`) und **Claude Haiku 4.5** (Anthropic, Modell-ID `claude-haiku-4-5-20251001`). Die Auswahl dieser Modelle begründet sich durch ihre im Vergleich zu den jeweiligen Vollmodellen reduzierte Latenz und Betriebskosten. Das System implementiert eine **Singleton-Provider-Architektur** mit automatischer Fallback-Kette: Zur Laufzeit wird der erste verfügbare Provider in der Reihenfolge OpenRouter → Gemini → OpenAI → Anthropic aktiviert — ist kein API-Schlüssel konfiguriert, wird auf einen regelbasierten Mock-Provider zurückgefallen. Moderne LLMs adressieren zudem die Mehrsprachigkeitsanforderung (Kapitel 2.2), da sie in zahlreichen Sprachen den bisherigen Stand der Technik übertreffen (OpenAI 2023, S. 1).
