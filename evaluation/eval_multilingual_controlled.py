#!/usr/bin/env python3
"""
Kontrollierter Mehrsprachigkeitsvergleich – DE vs. ES, gleiche Bedingungen
===========================================================================
Identische Bedingungen für beide Sprachen:
  - Temperatur: T=0.5 (wie DE-Hauptevaluation)
  - Few-Shot:   KEINE (für beide Sprachen)

Zweck: Isoliert den reinen Spracheffekt, bereinigt den Konfound aus
       eval_multilingual.py (DE: T=0.5+FS, ES: T=0.1, kein FS).

Nutzung:
  cd evaluation
  python3 eval_multilingual_controlled.py
  python3 eval_multilingual_controlled.py --n-per-type 2 --fixtures 3
"""
...
# ── Prompt Builder (kein Few-Shot für beide Sprachen) ─────────────────────────
...
def build_prompt(ev: dict, style: str, lang: str) -> str:
    ...
    if lang == "de":
        ...
        return (
            f"Du bist ein Liveticker-Redakteur. {lang_instruction}\n\n"
...
TEMPERATURE = 0.5  # identisch für beide Sprachen
...
def main():
    ...
    parser.add_argument("--n-per-type", type=int, default=2,  help="Events pro Ereignistyp (default 2 → ~30 Paare)")
    parser.add_argument("--fixtures",   type=int, default=3,  help="Spiele pro Liga")
    parser.add_argument("--delay",      type=float, default=1.2, help="Pause zwischen LLM-Calls (s)")
    parser.add_argument("--output",     default="eval_multilingual_controlled")
    args = parser.parse_args()

    print(f"Kontrollierter Mehrsprachigkeitsvergleich")
    print(f"Bedingungen: T={TEMPERATURE} (beide Sprachen), kein Few-Shot (beide Sprachen)")
    print(f"Modell: {GEN_MODEL}\n")
    ...
    for ev in selected:
        for style in STYLES:
            uid = f"ctrl_{ev['fixture_id']}_{ev['event_type']}_{ev['minute']}_{ev.get('player','')[:5]}_{style}"
            if uid in done_ids:
                print(f"  SKIP {uid[:60]}")
                continue
    ...
    if valid:
        print(f"\n{'='*65}")
        print(f"KONTROLLIERTER VERGLEICH (T={TEMPERATURE}, kein Few-Shot, N={len(valid)} Paare)")
        print(f"{'='*65}")
        ...
        for style in STYLES:
            ...
            if rows:
                ...
                es = round(sum(r["avg_es"]    for r in rows) / len(rows), 2)
