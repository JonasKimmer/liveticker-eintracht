...
1. **Technische Qualitätssicherung** (Abschnitt 6.2): Korrektheit der Kernlogik (Unit-Tests), Zusammenspiel der API-Schichten (Integrations-Tests), Stabilität der Benutzeroberfläche (End-to-End-Tests) und statische Typsicherheit (TypeScript-Coverage).
2. **Evaluation der KI-Textgenerierung** (Abschnitte 6.3–6.4): Quantitative Metriken, qualitative Analyse der generierten Texte und Vergleich der Betriebsmodi.
3. **Systembetrachtung** (Abschnitte 6.5–6.7): Performance und Laufzeitverhalten, Anforderungsabgleich (Soll-Ist) sowie Limitationen und offene Punkte.
...
**Coverage-Verteilung nach Schicht**
...
_Messgrundlage: N = 16 deutschsprachige KI-generierte Einträge (OpenRouter), N = 7 (Mock), gemessen über 28 FullTime-Spiele auf dem Render-Deployment. Qualitätswerte (1–5) wurden manuell anhand der Kriterien aus Abschnitt 6.3.6 bewertet._
...
Jede Dimension wurde auf einer Skala von 1 (ungenügend) bis 5 (exzellent) bewertet. Die Bewertung erfolgte durch den Entwickler; ein zweiter Rater stand nicht zur Verfügung, sodass kein Cohen's Kappa berechnet wurde. Die externe Validierung erfolgt durch das Experteninterview (s. u.).
...
Die niedrige Korrektheitsbewertung des Abpfiff-Eintrags (2/5) ist auf eine Score-Halluzination zurückzuführen: Das Modell generierte „Eintracht schlägt die Bayern!" trotz eines Spielstands von 1:2 (Niederlage) im Match-Kontext. Diese Fehlerklasse ist nachfolgend dokumentiert.

**Typische Stärken**

_Beispiel 1 — Tor (euphorisch, Bewertung 5/5/5):_
...
_Beispiel 2 — Rote Karte (euphorisch, Bewertung 5/5/5):_
...
_Beispiel 3 — Wechsel (neutral, Bewertung 4/4/4):_
...
_Beispiel 4 — Tor (Englisch, euphorisch, Übersetzung via translate_ticker_text):_
...
**Typische Schwächen und Fehlerklassen**
...
**Experteninterview: Praxisvalidierung durch professionellen Redakteur**
...
- _IF10: Wie bewerten Sie die Qualität der KI-generierten Ticker-Texte im Vergleich zu manuell verfassten?_
- _IF11: Lesen sich die Texte wie ein typischer Eintracht-Frankfurt-Ticker?_
- _IF12: Welchen der drei Modi (auto/coop/manual) würden Sie im Redaktionsalltag bevorzugt einsetzen?_
- _IF13: Ist der Coop-Modus — also KI-Entwurf mit redaktioneller Freigabe — ein realistischer Kompromiss zwischen Zeitersparnis und Qualitätskontrolle?_
- _IF14: Welche Features fehlen für einen produktiven Einsatz?_
- \_IF15: Erkennen Sie den vereinsspezifischen Eintracht-Stil in den generierten Texten wieder?
...
**Zentrale Erkenntnisse** — Das Interview findet im Rahmen der Abschlussphase dieser Arbeit statt; die Ergebnisse werden nachfolgend ergänzt.

- **Textqualität**: [Ergebnisse werden nach Durchführung ergänzt]
- **Praktikabilität**: [Ergebnisse werden nach Durchführung ergänzt]
- **Verbesserungsvorschläge**: [Ergebnisse werden nach Durchführung ergänzt]
- **Akzeptanz**: [Ergebnisse werden nach Durchführung ergänzt]
...
_TTP-Schätzungen basieren auf den Latenzmessungen aus Abschnitt 6.5.1 (Median: 859 ms) und dem Polling-Intervall von 5.000 ms (vgl. 6.5.3). Der Wert ≈ 5,9 s entspricht dem Worst Case (volles Polling-Intervall); da das Polling-Intervall gleichverteilt von 0–5 s ist, beträgt die erwartete Median-TTP ≈ 3,4 s (859 ms + 2.500 ms Erwartungswert). Im Vergleich zur manuellen TTP von 30–120 s ergibt sich ein geschätzter **Reduktionsfaktor von 5× bis 35×** (`auto`-Modus). Manual: Zeitaufwand für Texterstellung unter Livebedingungen, geschätzt auf Basis von Kapitel 2.1._
...
| N2  | Retry-Logik mit Rate-Limit-Erkennung   | ✅     | 3 Versuche; 30s/60s/90s bei Rate-Limit (`LLM_RATE_LIMIT_WAIT_BASE_S × attempt`); 1s/2s/4s bei sonstigen Fehlern (`2^attempt`) |
...
**Eingeschränkte Stichprobengröße.** 9 Spiele und 16 Events bilden die Datenbasis der qualitativen Evaluation. Für eine statistisch belastbare Aussage wäre eine größere Stichprobe über verschiedene Wettbewerbe, Ligen und Spielsituationen erforderlich.

**Kein Langzeitbetrieb.** Das System wurde nicht über einen längeren Zeitraum (z. B. eine vollständige Saison) im Produktivbetrieb evaluiert. Aspekte wie API-Kosten, Rate-Limit-Häufigkeit im Regelbetrieb und Drift der Textqualität über die Zeit sind daher nicht erfasst.

**Keine Authentifizierung.** Das System verzichtet bewusst auf eine Authentifizierungsschicht (vgl. Kapitel 4.2.6). Für einen produktiven Einsatz ist die Absicherung der API-Endpunkte — insbesondere der schreibenden Operationen (Publish, Reject, Retract, Mode-Wechsel) — zwingend erforderlich.

**Eingeschränkte E2E-Testabdeckung.** Die 6 Playwright-Tests decken den stabilen Kern der UI ab, testen aber nicht den vollständigen Redaktionsworkflow mit Backend-Anbindung (Spiel auswählen → Events empfangen → Ticker-Einträge bearbeiten → Freigeben). Ein vollständiger E2E-Test mit laufendem Backend und Testdatenbank wäre wünschenswert, wurde aber aufgrund der Komplexität des Setups nicht umgesetzt.

**Polling statt Push.** Die Frontend-Datenaktualisierung basiert auf Polling (5-Sekunden-Intervall). In einem produktiven Szenario mit vielen gleichzeitigen Nutzern wäre Server-Sent Events (SSE) oder WebSocket für die Ticker-Updates effizienter. Die WebSocket-Infrastruktur existiert bereits für den Media-Kanal (Clip-Import), wurde aber nicht auf den Ticker-Datenfluss ausgeweitet.

**Few-Shot-Reichweite.** Die Few-Shot-Infrastruktur wirkt nur bei Generierung über den Backend-LLM-Service. Die n8n-Workflows für Halbzeit- und Abpfiff-Zusammenfassungen rufen OpenRouter direkt auf und erhalten daher keine stilistischen Stilreferenzen. Zudem fällt der Liga-Filter bei zu wenigen liga-spezifischen Einträgen in der `style_references`-Tabelle auf eine liga-agnostische Suche zurück (der Filter selbst ist case-insensitiv über `func.lower()` implementiert). Das Few-Shot-System funktioniert damit grundsätzlich, liefert aber noch keine konsequent liga-spezifische Stilkonditionierung.

**Batch-Übersetzung schließt manuelle Einträge ein.** Der Endpunkt `POST /api/v1/ticker/translate-batch/{match_id}` übersetzt alle publizierten Einträge eines Spiels — unabhängig vom `source`-Feld. Das bedeutet, dass auch manuell verfasste Einträge übersetzt werden, obwohl diese bereits in der Zielsprache vorliegen könnten. Die Einschränkung liegt in einer fehlenden Filterung nach `source="ai"` im Batch-Endpunkt. Für eine präzise Mehrsprachigkeitsstrategie wäre eine separate Behandlung manueller Einträge sinnvoll.

**Keine Tests für n8n-Workflows.** Die 15 n8n-Workflows (vgl. Kapitel 5.5) verfügen über keine automatisierten Tests. Ihre Korrektheit wurde ausschließlich durch manuelle Ausführung und Inspektion der Datenbank-Ergebnisse verifiziert. Eine testgetriebene Absicherung wäre über n8n-Workflow-Mocks oder Integrationstests gegen eine Staging-Datenbank möglich, wurde aber aufgrund der visuellen Natur der n8n-Entwicklungsumgebung nicht umgesetzt.
